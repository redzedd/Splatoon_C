...
- **Why a fixed centre reticle needs a downward default pitch:** with a limited-range lobbed weapon, "does the shot land on the crosshair" is decided by CAMERA PITCH, not by the weapon. Measured sweep (2026-09-02, ranges from player): pitch 6° → crosshair at infinity vs shot 13.1m; 12° → 13.1m vs 7.4m; 18° → 8.6 vs 6.4; **24° → 6.3 vs 6.1 (matched)**; 30° → 4.8 vs 3.6. Hence default `_pitch = 22f` and `_minPitch = -15` (was 10 / -30) — the player starts inside the band where crosshair == impact. Verified at default view: crosshair 6.9m, mean impact 6.5m (`DefaultAimProbe`). NOTE camera SmoothDamp means a single `SetAngles` + one `LateUpdate` call reads a STALE camera — sweep probes must run as an in-game coroutine and wait ~30 frames per angle.
- **Ballistics — two-stage, NOT a single arc (user-observed Splatoon model, 2026-09-02):** inside `StraightRange` (10m) the shot flies nearly flat under `StraightGravity` (-1.5) so it holds the crosshair's height; past that range `DropGravity` (-38) takes over and it drops fast. Measured: 0.18m drop over the first 8m (the old single-gravity arc lost 1.5m+). **Ground trails are NOT drawn by regular drips** (that was a wrong guess, now deleted) — they emerge from two sources: every shot paints a muzzle splash at the player's feet (`InkShooter.PaintMuzzleSplash`, 0.42m at 1.3m ahead), and `EarlyDropChance` (18%) of shots get a randomly shortened range (2–7.5m) so sustained fire scatters ink along the path. `AimReticle` must mirror the same two-stage integration or its prediction diverges.
...
- **Tests:** 49 EditMode tests green (`Assets/Tests/EditMode/`) — pure logic + NUnit template; copy this pattern for all new pure logic. Play-mode: `LocomotionAutoTest` / `PaintAutoTest` / `ShootingAutoTest` / `SquidCoverageAutoTest` / `WallPaintAutoTest` / `ClimbAutoTest` / `InkTankAutoTest` / `FeelAutoTest` (`[AUTOTEST]` markers; aim/move cases point at open ground -Z — obstacles at +Z/+X create false results; see session-purity note above).
