...
- **Squid form (M1 step 5, done):** `SquidController` on Player (hold Crouch): own-ink ×1.8 / dry ×0.7 speed via solver's speedMultiplier, visual squash, blocks InkShooter. Ground-ink lookup = `InkWorld` singleton (scene GO GameSystems) wrapping Core `InkOwnershipGrid` (CPU coarse grid, marked synchronously by PaintableSurface.Paint — horizontal-ground assumption, revisit for walls).
