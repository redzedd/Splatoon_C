...
- **Ink tank (M3 step 1, done):** Core `InkTank` (0..1 normalized, whole-shot consume with float-epsilon tolerance, rate-based refill) owned by `PlayerInkTank` on Player (refill rate = squid-on-own-ink fast / standing slow, from LocomotionConfig 墨量 section; `InfiniteInk` debug flag — AutoPerfRun turns it on for FPS runs). **Refill runs every frame INCLUDING while firing, so sustained-fire duration is set by the NET rate, not by cost alone:** 16 shots/s × 0.01875 = 0.3/s out, standing refill 0.2/s in → net 0.1/s → a full tank lasts exactly 10s (user-specified target 2026-09-02, `InkTankAutoTest` measured 0.481 at 5s / 0.007 at 10.5s). Standing refill 0→full = 5s; squid-on-own-ink stays faster at 0.5/s (2s). Retune the cost and the refill together or the 10s target silently drifts. InkShooter consumes per shot (dry trigger = no projectile, FireClock rhythm unaffected); cost in WeaponConfig. HUD: `InkHud` + InkBarBG/InkBarFill on HudCanvas (fill width = ratio × 240, no sprite-fill machinery).
...
- **Shooting (M1 step 4, done):** `InkShooter` on Player (aim = camera-center ray, cone spread) + pooled `InkProjectile` (manual ballistic integration + segment raycast, `UnityEngine.Pool.ObjectPool`, prewarmed) + `FireClock` (Core, pure — call every frame including released; see its 呼叫契約 comment). Config: `Assets/Data/WeaponConfig.asset` (current tuning: fireInterval 0.0625 = 16 shots/s, muzzleSpeed 39, straightRange 10, splatRadius 1.82, splashRadius 0.84, muzzleSplashRadius 1.18, dripRadius 1.82, spread 2.5°, inkCostPerShot 0.01875 — the user scaled sizes ×1.4 then ×2 and fire rate ×2 on 2026-09-02; prefab scale 0.62 / trail width 0.34 alongside).
