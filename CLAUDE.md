...
- **Ballistics — two-stage, NOT a single arc (user-observed Splatoon model, 2026-09-02):** inside `StraightRange` (10m) the shot flies nearly flat under `StraightGravity` (-1.5) so it holds the crosshair's height; past that range `DropGravity` (-60) takes over and it drops fast. Measured: 0.00m drop over the first 8m (the old single-gravity arc lost 1.5m+). **Ground trails are NOT drawn by regular drips** (that was a wrong guess, now deleted) — they emerge from two sources: every shot paints a muzzle splash at the player's feet (`InkShooter.PaintMuzzleSplash`, 0.59m at 1.3m ahead), and `EarlyDropChance` (18%) of shots get a randomly shortened range (2–7.5m) so sustained fire scatters ink along the path. `AimReticle` must mirror the same two-stage integration or its prediction diverges.
