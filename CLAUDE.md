...
- **Aiming & TPS framing (M3 feedback pass, 2026-09-02):** camera pivot is lifted by `_aimHeightOffset` (1.1m) so the character sits in the lower screen half. The crosshair is NOT screen-center: `AimReticle` (on HudCanvas) re-simulates the projectile arc each LateUpdate (same integration as InkProjectile, via `InkShooter.TryComputeAim`) and moves the reticle to the predicted landing point — mandatory for a lobbed weapon (center-screen aim was 18.8m out while shots landed at 5.8m). Verified: predicted vs actual landing within 0.56m (`AimAutoTest`).
- **Weapon presentation:** visible `GunBarrel` + `Muzzle` node under Visual — offsets must exceed the capsule radius (0.5) or the barrel renders INSIDE the body (x=0.28 was invisible; 0.52 works). Firing forces the visual to face camera yaw (PlayerLocomotion), otherwise muzzle and crosshair disagree. Projectiles drip ink along the flight path (`InkProjectile.DripAlongPath`, spacing 0.6m / radius 0.35m — spacing must be < 2×radius for a continuous trail).
