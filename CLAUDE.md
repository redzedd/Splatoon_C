...
- **Tests:** 41 EditMode tests green (`Assets/Tests/EditMode/`) — pure logic + NUnit template; copy this pattern for all new pure logic. Play-mode: `LocomotionAutoTest` / `PaintAutoTest` / `ShootingAutoTest` / `SquidCoverageAutoTest` / `WallPaintAutoTest` / `ClimbAutoTest` (`[AUTOTEST]` markers; aim/move cases point at open ground -Z — obstacles at +Z/+X create false results).
