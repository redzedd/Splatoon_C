...
3.18 ✅ **墨路腰身修正**(2026-09-03,使用者回報「靠近自己 1/4 處常斷掉」):
   - 舊測試驗不出來:`RoadAutoTest` 用 ±0.6m 的窗、任一點有墨就算過,量的是「有沒有」;玩家看到的斷點其實是「路細到快斷」。
   - 新增 `RoadProfileProbe` 量**寬度剖面**(橫向 ±1.5m、13 點/距離),抓到 pitch 8° 在 **6.5m 處只有 1.50m 寬**且連續四段偏細;另發現 pitch 20°(一般俯視)時**腳前 1.0m 完全沒墨**。
   - 成因:路是一串圓,兩顆之間的腰身寬 = 2√(r² − (間距/2)²)。滴墨間距 1.775m 對半徑 1.2 → 腰身 1.50m。而槍口噴濺的距離是從**槍口**起算,槍口本身已在身前約 1.05m,所以最近的噴濺落在 2.35m 外。
   - 修法:`DripsPerShot` 2 → **3**(不同位置 8 → 12,間距 1.775 → **1.18m**);`MuzzleSplashDistance` 1.3 → **0.4**。
   - 結果:最細處 1.50 → **2.00m**;pitch 8° 的 1.0~4.5m 全部滿格;pitch 20° 腳前 1.0m 由 0.00 → 2.00m。
   - `RoadAutoTest` 新增案 4「路寬不出現腰身」(最細 ≥1.25m),以後這類問題會被測試擋下。
   - 驗證:Road 4/4、Shooting 3/3、Aim 5/5、滴墨池維持 96 無執行期 Instantiate。

3.19 ✅ **墨路真正的破洞:滴墨前飛距離失控**(2026-09-03,使用者附截圖):
   - 上一輪只修了「腰身」,截圖顯示仍有整段灰色空白,而且是**連續射擊也補不起來**——代表破洞在固定距離上,每發都在同一處漏。
   - 新增 `DripLandingProbe` 量**落點**(不是釋放點)的每公尺直方圖,一眼看出洞:pitch 0° 的 `.........21121.114232` —— 0~8m 完全沒有墨滴落地,而槍口噴濺只到 5.2m。
   - 真因:`DripInheritSpeed` 是**比例**(0.25)。當初定值時彈速 26(前飛約 2m);彈速一路調到 65.81 後,同樣的 0.25 讓墨滴前飛 **5~8m**,於是「釋放 0.8~15m」的排程變成「落點 9~20m」,中間 5~9m 沒人塗。與先前「初速與阻力必須成對調」是同一類耦合。
   - 修法:`DripInheritSpeed` 0.25 → **0.05**(前飛 6.58 → 1.32m),落點重新貼合排程。
   - 結果:落點直方圖補滿(`.1.3.12111312121`)、路寬剖面 pitch 8° 的 1.0~6.0m 全部滿格、最細 2.00m、Road 4/4、截圖確認 0.5 秒連射整條連續。
