...
3.16 ✅ **落點回歸**(2026-09-03,使用者回報「打的地方變得準心偏上」):
   - 先量再修:新增 `AimBiasProbe`(每站 5 發、取平均、**帶正負號**,同時報螢幕像素偏移;地面 4 個俯角 + 牆面 4 個距離)。結果證明**瞄準本身沒有偏移**——牆面 −0.02~−0.08m、地面 ±0.2m,若有偏也是偏下。
   - 真因是上一輪 ×1.35 初速的副作用:墜落段滑行 = 初速 ÷ 水平阻力,射程 15.9 → 18.2m。俯視時「更遠」在畫面上就是「更高」。
   - 修法(使用者選「射程調回原本」):阻力等比放大 7 → **9.45**,滑行維持 6.96m。子彈視覺上仍是 1.35 倍快,落點回到原位。射程 **16.3m**(原 15.9m,差 0.4m 在單發雜訊內)。
   - 驗證:RangeByPitchProbe 16.3/15.3/13.5/10.2/6.5m 單調遞減、AimBiasProbe 偏差全在 ±0.2m、Road 3/3、Shooting 3/3、Aim 5/5。
   - 教訓入 CLAUDE.md 兩則:初速與阻力必須成對調(落點由比值決定);取最佳值的探針天生驗不出系統性偏移。

3.17 ✅ **修正仰射時子彈直線衝上天**(2026-09-03,使用者回報):
   - 根因:墜落段的煞停只作用在 x/z,垂直速度完全沒煞。平射時 y 本來就接近 0 所以看不出來;仰射時子彈帶著 60+ m/s 向上,只有 −60 重力在拉,要一秒多才停,中間就是一條直線。
   - 量測(`UpwardShotProbe`,新增):−70° 仰射爬升 **40.1m**、沿彈道飛 **82.4m**,而射程設定只有 10m 直飛 + 約 7m 滑行 ≈ 17m,超出 5 倍。
   - 修法:墜落段對「向上」的垂直速度也套同一組指數煞停;向下不煞,急墜手感保留。`AimReticle` 同步鏡像(預測積分必須一致的鐵律)。
   - 修後:爬升 −30°/−50°/−70° = 8.2 / 11.5 / 13.9m,總飛行 25.1 / 27.8 / 30.0m。
   - 驗證:平射射程不受影響(16.3 → 16.2m)、RangeByPitchProbe 仍單調遞減、Aim 5/5、Road 3/3、Shooting 3/3。
   - 過程失誤:第一次量測時我在 play mode 進行中做 `Assets/Refresh`,domain reload 把 InkShooter 的物件池欄位清成 null,連噴 4 次 NRE。改腳本前一定要先停 play mode。
